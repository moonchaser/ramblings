...
class Dashing.Rickshawgraph extends Dashing.Widget
  ...
  _createGraph: ->
    ...
    y_axis = new Rickshaw.Graph.Axis.Y(graph: graph, ticks:6, tickFormat: formatNumber)
    ...
    if @get("hover")
      hoverDetail = new Rickshaw.Graph.HoverDetail { graph: graph, yFormatter: formatNumber }

    if @get("shelving")
      shelving = new Rickshaw.Graph.Behavior.Series.Toggle { graph: graph, legend: legend }

    if @get("order")
      order = new Rickshaw.Graph.Behavior.Series.Order( { graph: graph,	legend: legend } );
